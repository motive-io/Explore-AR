...
public class EpisodePurchaseManager : Singleton<EpisodePurchaseManager>, IStoreListener
{
    ...
    public EventHandler<ItemPurchasedEventArgs> ItemPurchased;
	public EventHandler InitializeComplete;
    public EventHandler InitializeFailed;
    ...
    public void Initialize()
    {
		...
		var items = EpisodeManager.Instance.GetEpisodes().Where(item => !string.IsNullOrEmpty(item.ProductIdentifier));
    ...
    public EpisodeSelectState GetEpisodeState(ScriptDirectoryItem episode)
    {
        ...
        var isRunning = EpisodeManager.Instance.IsRunning(episode);
        if (isRunning)
        {
